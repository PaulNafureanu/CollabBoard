...
export const makeUserRepo = (db: TxClient) => {
  ...
  const createAnonUserTx = async (): Promise<PublicUserType> => {
    const base = await db.user.create({ data: {}, select: publicUserSelect });
  ...
  return { findById, createAnonUser: createAnonUserTx, createPermUser, updateUser, deleteUser };
};
